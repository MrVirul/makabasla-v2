...
export default function AdminDashboard() {
  ...
  const fetchData = useCallback(async () => {
    console.info("[Dashboard] Starting data fetch. Token present:", !!accessToken, "Status:", status);
    setError(null);
    setLoading(true);
    ...
    const [vehicleResult, billingResult] = await Promise.allSettled([
      ...
      fetch(`${API_BASE}/vehicles`, { headers }).then(async (res) => {
        ...
        if (!res.ok) {
          ...
          throw new Error(`Vehicles fetch failed (${res.status}): ${text.slice(0, 200)}`);
        }
        const raw = await res.json();
        console.info("[Dashboard] Vehicles raw — isArray:", Array.isArray(raw), "length:", Array.isArray(raw) ? raw.length : "n/a");
        return Array.isArray(raw) ? (raw as Vehicle[]) : [];
      }),

      // ── All billings from the internal aggregate REST endpoint ──
      fetch(`${BILLING_BASE}/internal/billings`, { headers }).then(async (res) => {
        console.info("[Dashboard] Billings response status:", res.status);
        if (!res.ok) {
          const text = await res.text();
          throw new Error(`Billings fetch failed (${res.status}): ${text.slice(0, 200)}`);
        }
        const raw = await res.json();
        console.info("[Dashboard] Billings raw — isArray:", Array.isArray(raw), "length:", Array.isArray(raw) ? raw.length : "n/a");
    ...
    if (vehicleResult.status === "fulfilled") {
      console.info(`[Dashboard] ✅ ${vehicleResult.value.length} vehicles loaded.`);
    ...
    if (billingResult.status === "fulfilled") {
      console.info(`[Dashboard] ✅ ${billingResult.value.length} billing records loaded.`);
  ...
  const stats = useMemo(() => {
    ...
    const totalAdvances = billings.reduce((sum, b) => sum + b.total_advances, 0);
    ...
    return (
      <div className="p-12 space-y-12 animate-pulse bg-[#0B0B0B] min-h-screen">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map(i => <div key={i} className="h-32 bg-white/5 rounded-md border border-white/5" />)}
        </div>
    ...
    <div className="max-w-[1400px] mx-auto p-12 space-y-12 bg-[#0B0B0B] min-h-screen text-[#D1D0C5] selection:bg-[#F5A623]/30">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-8 border-b border-white/5 pb-10">
        <div className="space-y-2">
          <h1 className="text-4xl font-medium tracking-tight text-white/90">Intelligence</h1>
        ...
        <div className="flex items-center gap-3">
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="outline" className="h-10 px-4 font-mono text-[10px] tracking-widest uppercase border-white/5 bg-white/[0.02] text-[#646669] hover:text-[#F5A623] hover:bg-white/5 active:scale-95 transition-all">
                <CalendarIcon className="mr-2 h-3.5 w-3.5" />
                {dateRange.from ? format(dateRange.from, "dd MMM") : "Start"} — {dateRange.to ? format(dateRange.to, "dd MMM") : "End"}
              </Button>
            </PopoverTrigger>
            <PopoverContent className="w-auto p-0 bg-[#0B0B0B] border-white/10 shadow-2xl" align="end">
              <Calendar
                mode="range"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="bg-[#141414] border-none shadow-sm group hover:bg-[#1A1A1A] transition-all">
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            <CardTitle className="text-[10px] font-mono tracking-[0.2em] uppercase text-[#646669]">Revenue</CardTitle>
          ...
          <CardContent>
            ...
            <div className="flex items-center gap-1.5 mt-2">
                ...
                <span className="text-[10px] text-green-500/80 font-mono">+12.4%</span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-[#141414] border-none shadow-sm group hover:bg-[#1A1A1A] transition-all">
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            <CardTitle className="text-[10px] font-mono tracking-[0.2em] uppercase text-[#646669]">Receivables</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-[10px] text-[#646669] font-mono mt-2 tracking-wide uppercase">Outstanding balance</p>
          </CardContent>
        </Card>

        <Card className="bg-[#141414] border-none shadow-sm group hover:bg-[#1A1A1A] transition-all">
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            <CardTitle className="text-[10px] font-mono tracking-[0.2em] uppercase text-[#646669]">Velocity</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-[10px] text-[#646669] font-mono mt-2 tracking-wide uppercase">Open billing cycles</p>
          </CardContent>
        </Card>

        <Card className="bg-[#141414] border-none shadow-sm group hover:bg-[#1A1A1A] transition-all">
          <CardHeader className="flex flex-row items-center justify-between pb-3">
            <CardTitle className="text-[10px] font-mono tracking-[0.2em] uppercase text-[#646669]">Fleet Size</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-[10px] text-[#646669] font-mono mt-2 tracking-wide uppercase">Assets in registry</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Tabs defaultValue="analytics" className="w-full space-y-10">
        ...
        <TabsContent value="analytics" className="space-y-8 m-0 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {/* Revenue Trend Chart */}
            <Card className="lg:col-span-2 bg-[#141414] border-none p-8">
              <CardHeader className="px-0 pt-0 mb-8">
                <CardTitle className="text-lg font-medium text-white/90">Revenue Trajectory</CardTitle>
                ...
                <div className="h-[350px] w-full min-h-0 min-w-0">
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData}>
                      ...
                      />
                      <Bar dataKey="revenue" fill="url(#barGradient)" radius={[2, 2, 0, 0]} />
                    </BarChart>
                  </ResponsiveContainer>
                </div>
              </CardContent>
            ...
            <Card className="bg-[#141414] border-none p-8">
              <CardHeader className="px-0 pt-0 mb-8">
                <CardTitle className="text-lg font-medium text-white/90">Composition</CardTitle>
                ...
                <div className="grid grid-cols-2 gap-x-8 gap-y-3 mt-4 w-full">
                    {distributionData.slice(0, 4).map((entry, index) => (
                        <div key={entry.name} className="flex items-center gap-2">
                            ...
                            <span className="text-[10px] font-mono text-[#646669] uppercase tracking-wider truncate">{entry.name}</span>
                            <span className="text-[10px] font-mono text-white/40 ml-auto">{Math.round((entry.value / (stats.totalVehicles || 1)) * 100)}%</span>
                        </div>
                    ))}
                </div>
              </CardContent>
        ...
        <TabsContent value="registry" className="m-0 animate-in fade-in slide-in-from-bottom-4 duration-700">
          <Card className="bg-[#141414] border-none p-8">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
                <div>
                  <CardTitle className="text-xl font-medium text-white/90">Financial Registry</CardTitle>
                  <CardDescription className="text-xs text-[#646669] font-mono uppercase tracking-[0.15em] mt-1">Audit log of all processed billing cycles</CardDescription>
                </div>
                ...
                <div className="flex items-center gap-3 w-full md:w-auto">
                   ...
                   <Button variant="outline" className="h-9 border-white/5 bg-white/[0.02] text-[#646669] hover:text-[#F5A623] active:scale-95 transition-all">
              ...
              <Table>
                <TableHeader className="bg-white/[0.02]">
                  <TableRow className="border-white/5 hover:bg-transparent">
                    <TableHead className="py-5 font-mono text-[10px] tracking-widest uppercase text-[#646669]">Session</TableHead>
                    <TableHead className="py-5 font-mono text-[10px] tracking-widest uppercase text-[#646669]">Vehicle Entity</TableHead>
                    <TableHead className="py-5 font-mono text-[10px] tracking-widest uppercase text-[#646669]">Principal</TableHead>
                    <TableHead className="py-5 font-mono text-[10px] tracking-widest uppercase text-[#646669]">Gross</TableHead>
                    <TableHead className="py-5 font-mono text-[10px] tracking-widest uppercase text-[#646669]">Advances</TableHead>
                    <TableHead className="py-5 font-mono text-[10px] tracking-widest uppercase text-[#646669]">Net Due</TableHead>
                    <TableHead className="py-5 font-mono text-[10px] tracking-widest uppercase text-right text-[#646669]">Analysis</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {billings.map((b) => {
                    ...
                    return (
                      <TableRow key={b.id} className="border-white/5 hover:bg-white/[0.03] transition-all group cursor-default">
                        <TableCell className="py-4 font-mono text-[10px] text-[#646669]">#{b.id.substring(0, 8).toUpperCase()}</TableCell>
                        <TableCell className="py-4">
                          <div className="text-xs font-medium text-white/80">{vehicle ? `${vehicle.make.toUpperCase()} ${vehicle.model.toUpperCase()}` : `SYS_ID_${b.vehicle_id}`}</div>
                          <div className="text-[10px] text-[#F5A623]/60 font-mono mt-1 tracking-wider">{vehicle?.plate_number}</div>
                        </TableCell>
                        <TableCell className="py-4 text-[10px] font-mono text-[#646669] uppercase tracking-tighter">{vehicle?.customer?.name || "unidentified"}</TableCell>
                        <TableCell className="py-4 text-xs font-mono text-white/70">${b.total_expenses.toLocaleString()}</TableCell>
                        <TableCell className="py-4 text-xs font-mono text-[#F5A623]/80">${b.total_advances.toLocaleString()}</TableCell>
                        <TableCell className="py-4 text-xs font-mono">
                           <span className={cn(
                              "px-2 py-0.5 rounded-full",
                              b.balance_due > 0 ? "bg-[#ca4754]/10 text-[#ca4754]" : "bg-green-500/10 text-green-500"
                           )}>
                              ${b.balance_due.toLocaleString()}
                           </span>
                        </TableCell>
                        <TableCell className="py-4 text-right">
                          <Button variant="ghost" size="sm" className="h-8 w-8 p-0 text-[#646669] group-hover:text-white transition-colors">
                  ...
                  {billings.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={7} className="h-40 text-center py-20">
                         ...
                         <p className="font-mono text-[10px] text-[#646669] tracking-widest uppercase">
                            no transactional data found in currently selected range
                         </p>
                      </TableCell>
                    </TableRow>
                  )}
      ...
      <div className="pt-12 border-t border-white/5 flex flex-col md:flex-row justify-between items-center gap-6 opacity-40 hover:opacity-100 transition-opacity duration-700">
        <div className="flex items-center gap-6">
            <div className="flex items-center gap-2">
                ...
                <p className="font-mono text-[9px] tracking-[0.2em] uppercase">Gateway: secure</p>
            </div>
            <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 rounded-full bg-blue-500" />
                <p className="font-mono text-[9px] tracking-[0.2em] uppercase">Shards: 8 active</p>
            </div>
        </div>
